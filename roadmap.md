...
- **2.1 列表能力补全**
  ...
  - [x] 补充列表边界条件测试（空列表弹出、多客户端并发等）。

- **2.2 集合能力增强（可选）**
  - [x] 增加 `SUNION` 读操作（包含命令解析、存储实现与集成测试 `sets_union_behaviour`）。
  - [x] 在基准工具中新增 `set_union` 分组，对 SUNION 行为做简单性能/功能回归。
  - [x] 视需要增加 `SINTER` 读操作（包含命令解析、存储实现与集成测试 `sets_intersection_behaviour`）。
  - [x] 在基准工具中新增 `set_intersection` 分组，对 SINTER 行为做简单性能/功能回归。
  - [x] 增加 `SDIFF` 读操作（包含命令解析、存储实现与集成测试 `sets_difference_behaviour`）。
  - [ ] 为集合操作补充更多交叉测试用例。
